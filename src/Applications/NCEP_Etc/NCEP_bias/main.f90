...
!   2008-07-08 todling   replace use mpi with mpeu mpif90
!   2011-11-16 guo       added a searching algorithm to match entries
!                        between satinfo (passed through diag_rad) and
!                        satbias_ang.in
!   2014-02-04 j.jin     added 3-point and 7-point smoothing for tmi low 
!                        and high frequency data.
!   2016-03-04 m.j. kim  check on tb_tz less than -1000; supposedly all-sky only
!
! usage:
!   input files:
...
!$$$
! use mpi
  use bias_mpeu, only: mpi_comm_world
...
! Declare local parameters
  ...
  integer,parameter:: max_pos_tmi = 70, max_pos_gmi = 74
...
! Declare local variables
  ...
  character(20):: satsensor0_j
  integer:: jchanum0_j
  real:: tlap0_j
  real,allocatable,dimension(:):: c_ang0_j
...
! Allocate and initialize data arrays
  ...
  allocate(satsensor0(jpch),jchanum0(jpch),tlap0(jpch),c_ang0(nstep,jpch),c_ang0_j(nstep))
...
! Read input satang file
  ...
  ich=0
  do
     read(lnangl,110,err=120,end=100) ich, &
     	satsensor0_j,jchanum0_j,tlap0_j,(c_ang0_j(i),i=1,nstep)
     j=j+1

     if(j>jpch) cycle		! j<=jpch is verified before save it.
...
100 continue
! jpch agreement between "sac.nl" and "satbias_ang.in" is verified.
  if(j/=jpch) goto 120
  if(ich/=jpch) goto 120
...
120 continue
  write(6,*)'***ERROR*** EOF or ERROR reading input "satbias_ang.in" file'
  write(6,*)'***ERROR*** JPCH of "sac.nl"         = ',jpch
  write(6,*)'***ERROR***    J of "satbias_ang.in" = ',j
  write(6,*)'***ERROR***  ICH of "satbias_ang.in" = ',ich
  ierror_code=99
...
130 continue
  ...
  ! build a searchable table of (satsensor,jchanum)
  call sischnTable_build(sac_table,satsensor0(:jpch),jchanum0(:jpch))
...
!       Extract satinfo relative index
	...
	! The original indexing method assumes that header_chan(:)%iochan 
	! given in diag_rad file (defined by the record sequence of satinfo
	! in GSI) is the same record sequence of satbias_ang.in.  This is
	! not the case, if an initial satbias_ang.in is from a different
	! source.  Matching between diag_rad data and satbias_ang.in record
	! by their unique (isis,chan) values then becomes necessary.

        do j=1,n_chan
           !! io_chan(j) = real( header_chan(j)%iochan, 4 )
	   io_chan(j) = sischnTable_locate(sac_table,satsens,header_chan(j)%nuchan)
        end do
	if(any(io_chan(1:n_chan)<=0) .or. any(io_chan(1:n_chan)>jpch)) then
           !write(6,*)'***ERROR*** mismatching channel numbers'
           !write(6,*)'  diag_rad= ',trim(diag_rad)
           !write(6,*)'  io_chan = ',io_chan(1:n_chan)

           write(6,'(a,2x,a)') '>>> ERROR <<<','mismatching channel numbers (nuchan,satsens)'
           write(6,'(a,2x,a)') '>>> ERROR <<<','file diag_rad = "'//trim(diag_rad)//'"'
           write(6,'(a,2x,a)') '>>> ERROR <<<','  j_chan io_chan  nuchan  satsens'
           do j=1,n_chan
	     if( io_chan(j)<=0 .or. io_chan(j)>jpch ) then
               write(6,'(a,2x,3i8,4x,a)') '>>> ERROR <<<',j,io_chan(j),header_chan(j)%nuchan,satsens
             endif
           end do
...
!             angle dependent bias.
              ...
              if(data_chan(j)%tb_tz .lt. -1000.) then
                  cycle loopc
              endif
...
!             done to increase the sample size for the statistics.
              ...
              else if (trim(obstype) =='tmi' .or. &
                       trim(obstype) =='gmi' ) then
                 if(trim(obstype) == 'tmi') then
                    max_pos = max_pos_tmi
                 else
                    max_pos = max_pos_gmi
                 endif
                 if (j <= 7 ) then
                    do i=ispot-1, ispot+1
                       if (i < 1 .or. i > max_pos) cycle
                       c_ang1(i,jj) = c_ang1(i,jj) + bias*errinv
                       csum(i,jj)   = csum(i,jj)   + errinv
                       count(i,jj)  = count(i,jj)  + one
                    end do
                 else  !channel 8 & 9
                    do i=ispot-3, ispot+3
                       if (i < 1 .or. i > max_pos) cycle
                       c_ang1(i,jj) = c_ang1(i,jj) + bias*errinv
                       csum(i,jj)   = csum(i,jj)   + errinv
                       count(i,jj)  = count(i,jj)  + one
...
! Deallocate data arrays
  ...
  deallocate(count)
  deallocate(satsensor0,jchanum0,tlap0,c_ang0,c_ang0_j)
