...
!   2017-03-10:  Knob to allow assimilation of total column-like variables.
!
! attributes:
!   language: f95
!
...
use statevec, only: ensmean_chunk, anal_chunk, avkmean_chunk, ensmean_chunk_prior
...
use params, only: sprd_tol, paoverpb_thresh, ndim, datapath, nanals,&
                  ...
                  iseed_perturbed_obs,lupd_obspace_serial,ensmean_roundoff_fix,&
                  do_avk,fso_cycling, letkf_flag
...
logical lastiter, kdgrid, kdobs, lncase
...
! reset ob error to account for gross errors 
  ...
  obsloop: do nobx=1,nobstot
      ...
      if (deterministic) then
         ! EnSRF.
         if (do_avk) then
             obinc_one = hpfht/(one+sqrt(oberrvaruse(nob)*hpfhtoberrinv))
         endif
...
!$omp parallel do schedule(dynamic,1) private(ii,i,nb,obt,nn,nnn,lnsig,kfgain,taper1,taperv)
          do ii=1,nf2 ! loop over nearby horiz grid points
             ...
             i = sresults1(ii)%idx
             if(oblnp(nob)<-10._r_single) then
               do nn=1,nlevs_pres
                 taperv(nn)=taper1
               end do
             else
             ...
             do nn=nn1,nn2
                ...
                if (taperv(nnn) > zero) then
                    ...
                    if (do_avk) then
                       ! update analysis averaging kernel
                       kfgain=taperv(nnn)
                       avkmean_chunk(i,nn,nb) = avkmean_chunk(i,nn,nb) + kfgain*obinc_one
                    endif
                end if
             end do
...
!$omp parallel do  schedule(dynamic,1) private(nob1,nob2,nob3,lnsig,obt,kfgain)
        do nob1=1,nf
           ...
           if(oblnp(nob)<-10._r_single) then
              ...
              lncase = lnsig < lnsigl(nob)
              lntaper= taper(lnsig*lnsiglinv)
           endif
           if (lncase .and. taper_disob(nob1) > zero) then
             obt = abs(obtime(nob)-obtime_chunk(nob2))
             if (obt < obtimel(nob)) then
               ! gain includes covariance localization.
               kfgain = taper_disob(nob1)* &
                        lntaper*taper(obt*obtimelinv)* &
