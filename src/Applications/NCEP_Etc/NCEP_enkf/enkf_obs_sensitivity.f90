...
!   loc_advection   - Compute movement of localization center according to
!                     the pseudo-advection
...
!   adloc_chunk - Coordinate of moved coordinate by the pseudo-advection
...
use params, only: fso_flag,latbound,adrate,evalft,nlevs,nanals,datestring, &
                  ...
                  nbackgrounds,fso_cnt_usedob_only
...
use enkf_obsmod, only: nobstot,nobs_conv,nobs_oz,nobs_sat,obtype,obloclat, &
                       ...
                       corrlengthsq,obtimel,oblnp,obloc,iused,obfit_post
...
use statevec, only: ensmean_chunk,analmean_chunk
...
private
public init_ob_sens,destroy_ob_sens,print_ob_sens,loc_advection, &
       read_ob_sens,obsense_kin,obsense_dry,obsense_moist,adloc_chunk
...
type obsense_info
  ...
  integer(i_kind) :: iused              ! Index to indicate which obs were used (1=used)
...
!$$$ end documentation block
 ...
 ! nobstot=nobsgood
  ...
  do nob=1,nobs_conv+nobs_oz
     ...
     obfit_post(nob) = real(indata%obfit_post,r_kind)
  ...
  do nob=1,nobstot
     ...
     if (nob > nobs_conv+nobs_oz) then
      if (obtype(nob)(1:3) == 'aod') then
         lnsigl(nob)=-1.e5_r_single!latval(deglat,lnsigcutoffnh,lnsigcutofftr,lnsigcutoffsh)
      else
...
!$$$ end documentation block
  ...
  if(fso_flag) then
     ...
     call mpi_reduce(obsense_kin,recbuf,nobstot,mpi_realkind,mpi_sum,0, &
          & mpi_comm_world,ierr)
     if(nproc == 0) then
        if (fso_cnt_usedob_only) then 
           do nob=1,nobstot
              if(iused(nob)==1) then 
                 obsense_kin(nob) = recbuf(nob)
              else
                 obsense_kin(nob) = zero
              endif
           enddo
        else
           obsense_kin(1:nobstot) = recbuf(1:nobstot)
        endif
     endif
     ...
     if(nproc == 0) then
        if (fso_cnt_usedob_only) then 
           do nob=1,nobstot
              if(iused(nob)==1) then 
                 obsense_dry(nob) = recbuf(nob)
              else
                 obsense_dry(nob) = zero
              endif
           enddo
        else
           obsense_dry(1:nobstot) = recbuf(1:nobstot)
        endif
     endif
     ...
     if(nproc == 0) then
        if (fso_cnt_usedob_only) then 
           do nob=1,nobstot
              if(iused(nob)==1) then 
                 obsense_moist(nob) = recbuf(nob)
              else
                 obsense_moist(nob) = zero
              endif
           enddo
        else
           obsense_moist(1:nobstot) = recbuf(1:nobstot)
        endif
     endif
     deallocate(recbuf)
  end if
  if(nproc /= 0) return
  ! If all three measures are zero it is likely due to ob lying
  ! outside the advected the localization domain; flag these 
  ! cases ... there is a small change that an eventual observation
  ! giving mach prec zero impact will be tossed, but it is very
  ! unlikely to have such ob cases.
  if (fso_cnt_usedob_only) then 
    do nob=1,nobstot
       if (iused(nob)==0) cycle ! care only about used obs
       if (obsense_kin  (nob) == zero .and. &
           obsense_dry  (nob) == zero .and. &
           obsense_moist(nob) == zero) then
           iused(nob) = -1
       endif
    enddo
  endif
  ...
  if (nobs_conv+nobs_oz > 0) then
     ...
     do nob=1,nobs_conv+nobs_oz
        ...
        if (letkf_flag) then
           outdata%obfit_post = 0.0_r_single
        else
           outdata%obfit_post = real(obfit_post(nob),r_single)
        endif
