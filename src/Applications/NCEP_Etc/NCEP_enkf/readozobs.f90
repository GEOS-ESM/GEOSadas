...
use params, only: nsats_oz,sattypes_oz,npefiles,expid
...
subroutine get_num_ozobs(obspath,datestring,num_obs_tot,id)
    ...
    integer(i_kind) iunit,jiter,ii,ireal,iint,irdim1,ioff0,idate,ios,nsat,n,k,ipe
        ...
        peloop: do ipe=0,npefiles
           ...
           if (npefiles .eq. 0) then
               ...
               if(trim(expid)=="NULL") then
                  ...
                  obsfile = trim(adjustl(obspath))//trim(adjustl(id))//'/'//"diag_"//trim(sattypes_oz(nsat))//"_ges."//datestring//'_'//trim(adjustl(id))
               endif
...
10         continue
           ...
           go to 10
20         continue
           print *,'error reading ',trim(obstype), ' file'
30         continue
           close(iunit)
...
subroutine get_ozobs_data(obspath, datestring, nobs_max, h_x, h_xnobc, x_obs, x_err, &
  ...
  integer(i_kind) iunit,jiter,ii,ireal,iint,irdim1,ioff0,idate,nob,n,ios,nobs_max,nsat,k
  integer(i_kind) iunit2,jiter2,nlevs2,idate2,iint2,ireal2,irdim12,ioff02,ii2,ipe
         ...
         if(twofiles)then
           if (npefiles .eq. 0) then
               ...
               if(trim(expid)=="NULL") then
                 ...
                 obsfile2 = trim(adjustl(obspath))//trim(adjustl(id2))//'/'//"diag_"//trim(sattypes_oz(nsat))//"_ges."//datestring//'_'//trim(adjustl(id2))
               endif
...
10       continue
         ...
         allocate(diagbuf(ireal,ii))
         allocate(rdiagbuf(irdim1,nlevs,ii))
         allocate(rdiagbuf2(irdim12,nlevs,ii))
         read(iunit,err=20,end=30) idiagbuf,diagbuf,rdiagbuf
         if(twofiles)then
           ...
           ! if pressure is zero, it's a total column ob so don't use it.
             ...
             if(nlevs>1) then
               x_press(nob) = pob(k)
                ...
                x_press(nob) = rdiagbuf(4,k,n) ! k=1, in this case
             endif
         ...
         go to 10
20       continue
         print *,'error reading ',trim(isis),' file'
30       continue
         close(iunit)
