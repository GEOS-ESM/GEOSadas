...
! Check to see if ods file already exists.
          if(found)then
             ...
             if( maxval(abs(odsm%data%time)) > 180 ) then
                write(6,'(a,1x,i6.6,1x,a,2x,2i6)') &
                        '98: At nhms= ', nhms(k), 'min/max time ', minval(odsm%data%time), maxval(odsm%data%time)
                call exit(98)
             endif
             ...
             if( maxval(abs(ods_struct(1)%data%time)) > 180 ) then
                write(6,'(a,1x,i6.6,1x,a,2x,2i6)') &
                        '99: At nhms= ', nhms(k), 'min/max time ', minval(ods_struct(1)%data%time), maxval(ods_struct(1)%data%time)
!               call exit(99)
                call ODS_Clean( ods_struct(2), rc )
                call ODS_Clean( odsm, rc )
                return
