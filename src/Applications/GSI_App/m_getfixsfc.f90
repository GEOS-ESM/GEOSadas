...
subroutine write_ (fname,bvars,plevs,lats,lons,rc, nymd,nhms,myid,root)
  implicit none
  character(len=*), intent(in)    :: fname ! input filename
...
! an ID for each one.
  ...
  integer :: ncid, dimids(NDIMS), start(NDIMS)
...
! column-major format.
  dimids =  (/ x_dimid, y_dimid, t_dimid /) !, z_dimid /)
  start  =  (/ 1, 1, 1 /)
...
!               delp:valid_range = -1.e+15f, 1.e+15f ;
