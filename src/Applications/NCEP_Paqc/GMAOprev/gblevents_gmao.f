...
C
C 2007-06-29  M.SIENKIEWICZ (GMAO) ADD 'PG4243' FLAG FUNCTION FROM
C             PREPDATA TO FLAG THE INDIAN RADIOSONDES.  DEFAULT:
C             'PG4243=TRUE' MEANS GIVE INDIAN RAWINSONDE MASS
C             REPORTS (BLOCKS 42 AND 43) QM=15.
C
C
...
C  ----------------------
      ...
      IF (PG4243.AND.TYP.EQ.120.AND.(STNID(1:2).EQ.'42'.OR.
     $     STNID(1:2).EQ.'43')) THEN
         INDIAN = 1

         WRITE(IUNITS,71) STNID
...
C  -----------------------------------------------------------------
      ...
      DO L=1,NLEV
         ...
         IF (INDIAN .GT. 0) THEN
            ...
            QEV_8(4,L) = RCD
            MAXQEV = L
            CYCLE             ! reject all mass, don't check further
         ENDIF
...
!BOP
!
! !IROUTINE: GBLEVN10 (GMAO) - read background field and load in arrays
!
! !INTERFACE:
...
! !INPUT PARAMETERS
      ...
      integer iunitf(2)           !  unit number array (not used for GMAO)
      integer idatep              !  NCEP date/time YYYYMMDDHH for
                                  !    requested background field
      integer im, jm              !  x,y dimensions for background grid -
                                  !  not used for GMAO (we use native grid)
      integer idrt

! !DESCRIPTION:  Replacement for NCEP GBLEVN10 (formerly GESRES) which
!                  reads a background grid (or in NCEP's case, spectral)
!                  file and fills in a lat/lon/p grid.
!
!                Instead of reading from specified unit number, this
!                  routine opens a 'bkg' HDF file (in G5 format) specified
!                  on the command line.
!
!
! !REVISION HISTORY:
!
! 01May2008    M.Sienkiewicz   Initial version
!
!EOP
!-----------------------------------------------------------------------
      ...
      call getarg(1,fname)
      print *,'GBLEVN_GMAO:  Reading background information from ',
     &     trim(fname)

      call dyn_get(fname, nymd, nhms, w_f, rc, vectype=ivect)

      if (rc .ne. 0) then
         print *,'GBLEVN_GMAO: error in dyn_get, rc= ',rc
         call errexit(rc)
      endif

!      print *,'nymd, nhms',nymd,nhms
      mydate8 = nymd*100 + nhms/10000
      if (mydate8 .ne. idatep) then
         print *,'GBLEVN_GMAO: dates do not match:',idatep, mydate8
         call errexit(21)
      endif
      ...
      dlat = w_f%grid%lat_del
      dlon = w_f%grid%lon_del

! Check if the first longitude is -180 (in G5 it would be).  If so
! we must flip the input grid longitudes (NCEP code works on 0-360 coords)
!
! The reason for doing this flip here (rather than specifying 'forceflip'
! in the call to 'dyn_get' is that we do not want to force a flip if
! someday the grid is changed to (0,360) or possibly something else (?)
!
      if ( abs(w_f%grid%lon_min + 180.0) .lt. 0.01 ) then
         allocate ( dum(imax) )
         call hflip2( w_f%phis, imax, jmax, dum )
         call hflip2( w_f%ps,   imax, jmax, dum )
         call hflip3( w_f%pt,   imax, jmax, kmax, dum )
         call hflip3( w_f%u,    imax, jmax, kmax, dum )
         call hflip3( w_f%v,    imax, jmax, kmax, dum )
         call hflip3( w_f%q,    imax, jmax, kmax, dum )
         deallocate ( dum )
      else if (abs(w_f%grid%lon_min) .ne. 0) then
         print *,'GBLEVN_GMAO: cannot handle grid with lon_min=',
     &      w_f%grid%lon_min,', exiting'
            call errexit(23)
      endif
     ...
     $         iarpsl(imax,jmax,kmax),iarpsi(imax,jmax,kmax+1))
      ...
      do j = 1,jmax
         do i = 1,imax
            iar12z(i,j) = w_f%phis(i,j)*gravinv   ! Orography
            iar13p(i,j) = w_f%ps(i,j) * 0.01      ! Sfc press (hPa)
         enddo
      enddo
      ...
      do ll = 1,kmax
         do j = 1,jmax
            do i = 1,imax
               l = kmax - ll + 1
               iar14t(i,j,ll) = w_f%pt(i,j,l)
               iar15u(i,j,ll) = w_f%u(i,j,l)
               iar16v(i,j,ll) = w_f%v(i,j,l)
               iar17q(i,j,ll) = max(0.0,w_f%q(i,j,l,1)*1.0e6)
            enddo
         enddo
      enddo
      ...
      do ll = 1,kmax+1
         do j = 1,jmax
            do i = 1,imax
               l = kmax - ll + 2
               iarpsi(i,j,ll) = (w_f%grid%ak(l)+
     &              w_f%grid%bk(l)*w_f%ps(i,j))*0.01
            enddo
         enddo
      enddo
      ...
      do j = 1,jmax
        do i = 1,imax
          do l = 1,kmax
            iarpsl(i,j,l)=((iarpsi(i,j,l)**kap1-iarpsi(i,j,l+1)**kap1)/
     &         (kap1*(iarpsi(i,j,l)-iarpsi(i,j,l+1))))**kapr
          enddo
        enddo
      enddo
      ...
      print *,' RETURNING from GBLEVN10'

      return
      end
      subroutine hflip3 ( q,im,jm,km,dum )
      implicit none
      integer  im,jm,km,i,j,k
      real, intent(inout) :: q(im,jm,km)
      real, intent(inout) :: dum(im)
      do k=1,km
      do j=1,jm
      do i=1,im/2
         dum(i) = q(i+im/2,j,k)
      ...
      enddo
      enddo
      end subroutine hflip3

      subroutine hflip2 ( q,im,jm,dum )
      implicit none
      integer  im,jm,i,j
