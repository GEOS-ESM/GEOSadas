#!/usr/bin/env perl
# 
#  setup_4r21c - overwrites setting from fvsetup based on GEOS-R21C configuration
#                 this is to be called from within fvsetup
#
#  Jul2021 EL Akkraoui  Initial code (mimic code from setup_4merra2.pl)
#
#-----------------------------------------------------------------------------------------------------
...
foreach $fn ( @rcana ) {
  cp("$FVETC/gsi/R21C/$fn","$MYRUN/$fn");
  print "copying $FVETC/gsi/R21C/$fn over to $MYRUN/$fn \n";
  if($fn =~ "gsi.rc.tmpl" ) {ed_gsi_rc($MYRUN,"gsi.rc.tmpl")};
  if($fn =~ "gsi_sens.rc.tmpl" ) {ed_gsi_rc($MYRUN,"gsi_sens.rc.tmpl")};
}

# Transfer ensemble resource files to proper location
foreach $fn ( @rcens ) {
  cp("$FVETC/atmens/R21C/$fn","$MYRUN/atmens/$fn");
  print "copying $FVETC/atmens/R21C/$fn over to $MYRUN/atmens/$fn \n";
}
...
if ( $nymd < 20000331 ) {
   if ( $hh < 23 ) {
      $thisdir = `pwd`;
      foreach $fn ( @rcgocart ) {
         cp("$FVETC/R21C/19600101-20000331/$fn","$MYRUN/gocart/$fn");
      }
   }
}
