package Create_o3anasa_script;
#########################################################################
#
# name - create_o3anasa_script.pm
#
# purpose - To create job script for stand-alone analysis
#
# !REVISION HISTORY:
#
# 17Feb2023  Todling   Initial based on anasa
#
#########################################################################
use strict;
use warnings;
...
require Exporter;
our @ISA = "Exporter";
our @EXPORT_OK = qw( o3anasa_script );
...
sub o3anasa_script {
 ...
 open(SCRIPT,">$fvhome/o3anasa/$jobsa.j") or
 die ">>> ERROR <<< cannot write $fvhome/o3anasa/$jobsa.j";
...
#$jobqueue1
##SBATCH --export=NONE
#SBATCH --job-name=o3anasa
#SBATCH --output=o3anasa.log.o%j
#SBATCH --ntasks=$ncpus_gsi
#SBATCH --ntasks-per-node=24
#SBATCH --constraint=$nodeflg
#SBATCH --time=1:30:00
#PBS -N o3anasa
#PBS -o o3anasa.log.o%j
...
# ----------------------------------------------
  setenv ACFTBIAS 0 # knob for aircraft bias correction (1=on; 0=off)  
...
# O3-specific obs-class
# ---------------------
  if ( ! (\$?OBSCLASS_O3) ) then
     echo "User should edit FVDAS_Run_Config in FVHOME/run and add "
     echo "   setenv OBSCLASS_O3  o3_specific_classes"
     echo " aborting ..."
     exit 1
  else
     setenv OBSCLASS \$OBSCLASS_O3
  endif
...
# -----------------------------------------------------------------
  if ( `uname -s` == "Linux" ) then
    set path = ( . \$FVHOME/o3anasa \$FVHOME/run \$FVROOT/bin \$FVROOT/lib/grads \$BASEDIR/\$ARCH/bin \$path )
  else
    set path = ( . \$FVHOME/o3anasa \$FVHOME/run \$FVROOT/bin \$FVROOT/lib/grads \$path )
  endif

  echo \$FVWORK > \$FVHOME/.O3ANASA # record working directory
...
#      --------------------
    ...
    o3anasa -obsclass \$OBSCLASS \$FVHOME \$FVWORK \$FVWORK \$nymd \$nhms \$EXPID
...
# ---------------------------------------------------------
  ...
  pesto -v -d \$FVWORK -arc o3anasastorage.arc -cp /bin/mv
...
# -------------------------------------------
  if ( !(\$?this_nymdhh) ) then   # only archive when submitting one analysis at a time
     if ( \$BATCH_SUBCMD == "sbatch" ) then
        sbatch -J o3anasa.archive -o o3anasa.archive.log.o%j --export=arch_type=O3ANASA \$FVHOME/run/fvarchive.j
     else
        qsub   -N o3anasa.archive -o o3anasa.archive.log.o%j -v arch_type=O3ANASA \$FVHOME/run/fvarchive.j
     endif
  endif
