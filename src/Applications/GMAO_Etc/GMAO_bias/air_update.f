...
!   17Nov2016 Sienkiewicz  add namelist array to choose KX values for bias correction accumulation
!
!EOP
!------------------------------------------------------------------------- 
      ...
      integer,dimension(maxkx):: kx_list, kx2chk
...
! default values for screening
 ...
 1235 continue
      ...
      nkx = 0             ! walk through kx_list, stop at 0 value
      do i = 1,maxkx
         if (kx_list(i) .le. 0) exit
         nkx = nkx + 1
         kx2chk(nkx) = kx_list(i)
      end do

      if (nkx .eq. 0) then  ! if no kx values, use defaults
         kx_list(1) = 131
         kx_list(2) = 133
         kx2chk = kx_list
         nkx = 2
...
!
      ...
      do while (ndat .lt. maxobs)
         ...
         do i = 1,nobs
c$$$            select case (nint(rdiagbuf(1,i)))
c$$$            case (130, 131, 133)
            if (any(nint(rdiagbuf(1,i))==kx2chk(1:nkx))) then
               ndat = ndat + 1
